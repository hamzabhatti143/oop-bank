...
class bankAccount{
    ...
    constructor(accountnumber: number, initialBalance: number = 0, owner: string){
...
class Bank{
    ...
    public createAccount(owner: string, accountNumber?: number, initialBalance: number = 0): bankAccount {
        if (!accountNumber) {
    ...
    public getAccount(accountNumber: number): bankAccount | undefined{
        return this.accounts.find(account => account.getAccountNumber() === accountNumber); 
    }

    public transfer(fromAccountNumber: number, toAccountNumber: number, amount: number): void{
